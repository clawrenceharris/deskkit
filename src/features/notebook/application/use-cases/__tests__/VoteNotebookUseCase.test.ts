...
describe("VoteNotebookUseCase", () => {
  ...
  it("creates or updates a vote for the current user", async () => {
    vi.mocked(getCurrentUser).mockResolvedValue(ok(createMockUser({ id: "user-1" })));
    const repository = makeRepository({
      vote: vi.fn().mockResolvedValue(undefined),
    });
    const useCase = new VoteNotebookUseCase(repository);

    const result = await useCase.execute({ notebookId: "notebook-1", userId: "user-1", isUpvote: true });
  ...
  it("removes a vote when the input is null", async () => {
    ...
    const result = await useCase.execute({ notebookId: "notebook-1", userId: "user-1", isUpvote: null });

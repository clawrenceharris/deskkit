...
import { MemberForDetail } from "@/features/desk/infrastructure/queries";
import { useDeskMemberPresence } from "../../hooks";
...
/**
 * ParticipantsList component displays members as floating avatar clouds.
 *
 * Features:
 ...
 * - Real-time member updates with smooth enter/exit animations
...
export function ParticipantsList({
  ...
  // Track active members using presence
  const { activeMembers, totalActiveCount } = useDeskMemberPresence(
    deskId,
    currentUserId || "",
    members
  );
  ...
  const getDisplayName = useCallback((member: MemberForDetail): string => {
    ...
    return (
      ...
      `User ${member.profile.userId.slice(0, 8)}`
    );
  },[currentUserId]);
  // Initialize positions for members
  useEffect(() => {
    if (!containerRef.current || activeMembers.length === 0) return;

    const container = containerRef.current;
    const rect = container.getBoundingClientRect();
    setContainerSize({ width: rect.width, height: rect.height });

    const displayedParticipants = activeMembers.slice(0, maxVisible);

    const newPositions: ParticipantPosition[] = [];

    displayedParticipants.forEach((member) => {
      const size = member.profile.userId === creatorId ? 60 : 48; // Host avatars are larger
      ...
      newPositions.push({
        ...
        size,
        color: generateGradientColor(member.profile.userId),
        username: getDisplayName(member),
        isHost: member.profile.userId === creatorId,
        isCurrentUser: member.profile.userId === currentUserId,
        avatarUrl: member.profile.avatarUrl,
      });
    });

    setPositions(newPositions);
  }, [activeMembers, creatorId, currentUserId, getDisplayName, maxVisible]);
    ...
    <div className="relative overflow-hidden">
      <div className="p-4 border-b">
        <h3 className="text-lg font-semibold flex items-center gap-2">
          ...
          {members.length > totalActiveCount && (
        ...
        {positions.map((pos) => (
          ...
          >
            ...
            <Avatar
              ...
              className={`
                relative w-full p-1 h-full rounded-full bg-linear-to-br ${
                  pos.color
                }
            ...
            >
              <AvatarImage src={pos.avatarUrl ?? undefined} />
              <AvatarFallback>

...
export async function getSchoolsAction(): Promise<ActionResult<School[]>> {
  try {
    const result = await makeSchoolReadService().getSchools();
...
export async function getDetailedSchoolsAction(): Promise<ActionResult<SchoolForDetail[]>> {
  try {
    const result = await makeSchoolReadService().getDetailedSchools();
    if (!result.success) return fail(toActionError(result.error));
    return ok(result.data);
  } catch (error) {
    return fail(toActionError(ApplicationError.unexpected(error)));
...
export async function getPolicySchoolsAction(): Promise<ActionResult<SchoolForPolicy[]>> {
  try {
    const result = await makeSchoolReadService().getPolicySchools();
    if (!result.success) return fail(toActionError(result.error));
    return ok(result.data);
  } catch (error) {
    return fail(toActionError(ApplicationError.unexpected(error)));

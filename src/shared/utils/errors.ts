...
export class ApplicationError extends Error {
  ...
  export const errorMessages: Record<AppErrorCode, string> = {
    // Authentication
    [AppErrorCode.AUTH_INVALID_CREDENTIALS]: "Oops — that login didn’t work. Check your email and password.",
    ...
    [AppErrorCode.USERNAME_ALREADY_EXISTS]: "This username already exists. Try a different one.",

    // Authorization
    [AppErrorCode.PERMISSION_DENIED]: "You don’t have permission to do that.",

    // Validation
    [AppErrorCode.VALIDATION_FAILED]: "Something didn’t look right. Please check your input.",
    [AppErrorCode.FILE_TOO_LARGE]: "That file is too large. Try a smaller one.",

    // Network
    [AppErrorCode.NETWORK_OFFLINE]: "No internet detected. Please check your connection.",
    [AppErrorCode.NETWORK_TIMEOUT]: "That request took too long. Please try again.",
    [AppErrorCode.NETWORK_SERVER_ERROR]: "Something broke on the server. Our team is looking at it.",
    [AppErrorCode.RATE_LIMITED]: "Too many requests. Please wait a moment and try again.",
    ...
    // External
    [AppErrorCode.EXTERNAL_SERVICE_ERROR]: "A service we depend on failed. Please try again soon.",

    // Generic
    [AppErrorCode.UNKNOWN_ERROR]: "Oops — something broke. Please stand by while we fix it.",
    [AppErrorCode.NOTEBOOK_TIMEOUT]: "That notebook took too long to load. Please try again.",
  };
  ...
  function normalizePrismaError(error: Prisma.PrismaClientKnownRequestError): ApplicationError {
  ...
  function normalizeSupabaseError(error: {
    ...
    if (normalizedMessage.includes("invalid login credentials")) {
      return new ApplicationError({ code: AppErrorCode.AUTH_INVALID_CREDENTIALS });
    }

    if (normalizedMessage.includes("email not confirmed")) {
      return new ApplicationError({code: AppErrorCode.AUTH_EMAIL_NOT_CONFIRMED});
    }

    if (normalizedMessage.includes("user already registered")) {
      return new ApplicationError({code: AppErrorCode.AUTH_EMAIL_ALREADY_EXISTS});
    }
